...
register_quote_modal_callback('product', _QUOTE_RECIPIENT_EMAIL)


def get_item_details(item_id):
    """Fetches details for a specific product item."""
    if not item_id:
        return None
    query = """
...
def get_related_items(item_id, category_id):
    """Fetches related products based on the same category."""
...
def layout(id=None, **kwargs):
    return html.Div([
        dcc.Location(id='product-detail-url', refresh=False),
...
@callback(
    ...
    Input('product-detail-url', 'search')
)
def update_product_detail_page(search):
    ...
    if not item:
        return html.Div([
            html.H2("Product Not Found", className="no-data-text"),
            dcc.Link("Back to Products", href="/product-list", className="cat-allubot-btn")
        ], className="categories-page-wrapper", style={"textAlign": "center", "padding": "50px"})

    raw_img = item.get("image_url")
    image_src = raw_img if raw_img and str(raw_img).strip() != "" else "/assets/icons/product_icon.svg"
    is_fallback = "product_icon" in str(image_src)
    img_class = "item-large-img product-icon" if is_fallback else "item-large-img"

    related_products = get_related_items(item["id"], item.get("category_id"))

    return html.Div(
        className="categories-page-wrapper product-detail-theme",
        children=[
            ...
            html.Div(
                ...
                children=[
                    ...
                    html.Button(
                        ...
                        id="allubot-trigger-product-details", n_clicks=0,
            ...
            html.Div(
                ...
                children=[
                    dcc.Link("Home", href="/"), " / ",
                    dcc.Link("Products", href="/product-list"), " / ",
                ...
                children=[
                    ...
                    html.Div(
                        ...
                        children=[
                            ...
                            html.Button(
                                "Request a Quote",
                                id='product-quote-open-btn', n_clicks=0,
                                className="item-action-btn product-btn"
                            )
                        ]
                    )
                ]
            ...
            # 8. RELATED PRODUCTS SECTION (Conditional)
            html.Div(
                className="related-section",
                style={"display": "block" if related_products else "none"},
                children=[
                    html.H2("Related Products", className="section-heading"),
                    html.Div(
                        className="items-grid-wrapper",
                        children=[
                            dcc.Link(
                                href=f"/product-details?id={rel['id']}",
                                ...
                                children=[
                                    html.Div(
                                        ...
                                        children=[
                                            html.Img(
                                                src=rel["image"], 
                                                className="item-card-img product-icon" if "product_icon" in str(rel["image"]) else "item-card-img", 
                                                alt=rel["title"]
                                            )
                                        ]
                                    ),
                ...
                ] if related_products else []
            ),

            # 9. REQUEST-A-QUOTE MODAL (hidden until the button above is clicked)
            render_quote_modal(
                prefix='product',
                item_name=item["name"],
